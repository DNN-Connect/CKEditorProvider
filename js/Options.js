...
function pageLoad() {
    ...
    var refreshMainPane = function(element) {
        var buttonId = element.attr('id');
        var mainControlId = 'ProviderConfiguration';
        var mainPanelId = buttonId.substr(0, buttonId.indexOf(mainControlId) + mainControlId.length) + '_UP';
        if (typeof $find != "undefined" && $find(mainPanelId)) {
            $find(mainPanelId).ajaxRequest();
        }
    };
    ...
    jQuery('#ExportDialog').dialog({
        ...
        buttons: {
            "Cancel": function() {
                ...
                jQuery(this).dialog("close");
                refreshMainPane(jQuery(".ExportHidden"));
            }
        },
        open: function () {
    ...
    jQuery('#ImportDialog').dialog({
        ...
        buttons: {
             "Cancel": function() {
                 ...
                 jQuery(this).dialog("close");
                 refreshMainPane(jQuery(".ImportHidden"));
             }
        },
        open: function () {
            jQuery(this).parent().appendTo("form");
