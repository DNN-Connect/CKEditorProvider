...
namespace ElFinder
{
    ...
    public class Root
    {
        ...
        /// </summary>
        /// <param name="directory">Directory which will be root</param>
  //      public Root(IFolderInfo directory) : this(directory, null) { }
        ...
        /// </summary>
        public IFolderInfo ThumbnailsStorage
        {
            ...
            set
            {
                if (value != null)
                {
                    _thumbnailsDirectory = new FolderInfo();
                    //Path.Combine(value.FullName, ".tmb_" + _directory.Name));
        ...
        #region private
        //internal Size GetImageDimension(IFileInfo input)
        //{
        //    if (!input.Exists)
        ...
        internal bool CanCreateThumbnail(IFileInfo input)
        {
            return false; // ThumbnailsUrl != null && PicturesEditor.CanProcessFile(input.Extension); TODO:
        }

        internal string GetExistingThumbPath(IFileInfo originalImage)
        {
            string thumbPath = GenerateThumbPath(originalImage);
            return thumbPath != null && File.Exists(thumbPath) ? thumbPath : null;
        }

        internal string GetExistingThumbPath(IFolderInfo originalDirectory)
        {
            if (_thumbnailsDirectory == null)
                return null;
            string relativePath = originalDirectory.PhysicalPath.Substring(_directory.PhysicalPath.Length);
            string thumbDir = _thumbnailsDirectory.PhysicalPath + relativePath;
            return System.IO.Directory.Exists(thumbDir) ? thumbDir : null;
        }

        internal string GetExistingThumbHash(IFileInfo originalImage)
        {
            return null;
            //string thumbPath = GetExistingThumbPath(originalImage); TODO:
            //if (thumbPath == null)
            //    return null;
            //string relativePath = thumbPath.Substring(_thumbnailsDirectory.PhysicalPath.Length);
            //return VolumeId + Helper.EncodePath(relativePath);
        }

        internal string GenerateThumbHash(IFileInfo originalImage)
        {
            return null;
            //if (_thumbnailsDirectory == null)TODO:
            //{
        ...
        {
            return null;
            //string name = originalImage.File.Name; TODO:
            ...
            //    else
            //        thumbPath = new IFileInfo(Path.Combine(_thumbnailsDirectory.FullName, originalImage.RelativePath));
            //    if (!thumbPath.Exists)
            //    {
        ...
        private string GenerateThumbPath(IFileInfo originalImage)
        {
//            if (_thumbnailsDirectory == null || !CanCreateThumbnail(originalImage))
                return null;
            //string relativePath = originalImage.FullName.Substring(_directory.FullName.Length); TODO:
        ...
        private IFolderInfo _directory;
        private IFolderInfo _startPath;
        ...
        private IFolderInfo _thumbnailsDirectory;
        private IFolderInfo _thumbnailsStorage;
